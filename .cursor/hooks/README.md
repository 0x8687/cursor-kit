...
**Configuration:** Already enabled in `.cursor/settings.json`
...
echo '{"tool_input":{"file_path":"/tmp/test-large.js"}}' | node .cursor/hooks/modularization-hook.js

# Test with small file (should be silent)
echo "console.log('test');" > /tmp/test-small.js
echo '{"tool_input":{"file_path":"/tmp/test-small.js"}}' | node .cursor/hooks/modularization-hook.js
```
...
**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[{"tool":"Read","parameters":{"file_path":"test.ts"}}]}' | ./.cursor/hooks/discord_notify.sh
```
...
**Quick Test:**
```bash
./.cursor/hooks/send-discord.sh 'Test notification'
```
...
**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[]}' | ./.cursor/hooks/telegram_notify.sh
```
...
**Usage:**
```bash
./.cursor/hooks/send-discord.sh 'Your message here'
```
...
**OR `.cursor/.env`** (for project-specific overrides):
```bash
# Same variables as above
```

**OR `.cursor/hooks/.env`** (for hook-only configuration):
```bash
# Same variables as above
```
...
Hooks are configured in `.cursor/settings.local.json`:
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "${CURSOR_PROJECT_DIR}/.cursor/hooks/telegram_notify.sh"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
- Verify `.cursor/settings.json` uses `node .cursor/hooks/scout-block.js`
- Test manually: `echo '{"tool_input":{"command":"ls node_modules"}}' | node .cursor/hooks/scout-block.js`
...
- Review [Claude Code Documentation](https://docs.cursor.com/claude-code)
...
- [Claude Code Hooks Reference](https://docs.cursor.com/claude-code/hooks)
