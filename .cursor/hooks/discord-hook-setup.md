...
2. **.cursor/.env** - Project-level Claude configuration
3. **.cursor/hooks/.env** - Hook-specific configuration
...
**Option B: `.cursor/.env`** (project-level override):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```

**Option C: `.cursor/hooks/.env`** (hook-specific):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```
...
```bash
chmod +x .cursor/hooks/send-discord.sh
```
...
```bash
./.cursor/hooks/send-discord.sh 'Test notification from Claude Code'
```
...
```bash
./.cursor/hooks/send-discord.sh 'Task completed: Added user authentication feature'
```

**With multi-line messages:**
```bash
./.cursor/hooks/send-discord.sh 'Implementation Complete
...
```bash
./.cursor/hooks/send-discord.sh 'Fixed bug in user'\''s profile page'
```

### Automated Usage (Claude Code Workflow)

Claude automatically calls this script when completing implementations. This is configured in `.cursor/workflows/development-rules.md`:

```markdown
- When you finish the implementation, send a full summary report to Discord channel
  with `./.cursor/hooks/send-discord.sh 'Your message here'` script
```
...
if npm run build && npm run test; then
    ./.cursor/hooks/send-discord.sh '✅ Build and tests passed - ready to deploy'
else
    ./.cursor/hooks/send-discord.sh '❌ Build or tests failed - deployment blocked'
fi
```
...
{
  "scripts": {
    "deploy": "npm run build && npm run test && ./.cursor/hooks/send-discord.sh '✅ Deployment successful'",
    "notify": "./.cursor/hooks/send-discord.sh"
  }
}
```
...
```bash
./.cursor/hooks/send-discord.sh 'Message with "double quotes" works fine'
```

**Escape single quotes inside single-quoted strings:**
```bash
./.cursor/hooks/send-discord.sh 'User'\''s profile updated'
```

**Use double quotes and escape:**
```bash
./.cursor/hooks/send-discord.sh "Message with \"escaped quotes\""
```

**For complex messages, use heredoc:**
```bash
./.cursor/hooks/send-discord.sh "$(cat <<'EOF'
Multi-line message
...
./.cursor/hooks/send-discord.sh "$1"
...
if npm run test; then
    if [[ "$ENVIRONMENT" == "production" ]]; then
        ./.cursor/hooks/send-discord.sh '✅ Production deployment successful'
    fi
else
    # Always notify on failures
    ./.cursor/hooks/send-discord.sh '❌ Tests failed - deployment aborted'
fi
```
...
**Script Location:** `.cursor/hooks/send-discord.sh`
...
**Claude Code Documentation:** https://docs.cursor.com/claude-code
