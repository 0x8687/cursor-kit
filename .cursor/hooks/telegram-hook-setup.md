...
#### Option C: `.cursor/.env` (Project-Level Override)
...
#### Option D: `.cursor/hooks/.env` (Hook-Specific)
...
- `${CURSOR_PROJECT_DIR}`: Environment variable for project directory path

### 5. Make Script Executable

```bash
chmod +x .cursor/hooks/telegram_notify.sh
```
...
}' | ./.cursor/hooks/telegram_notify.sh
```
...
1. **Verify `.cursor/config.json` exists and is valid JSON:**
   ```bash
...
2. **Check hook configuration:**
   ```bash
   cat .cursor/config.json | jq '.hooks'
   ```

3. **Verify script is executable:**
   ```bash
   ls -l .cursor/hooks/telegram_notify.sh
...
**Script Location:** `.cursor/hooks/telegram_notify.sh`
...
**Claude Code Hooks:** https://docs.cursor.com/claude-code/hooks
