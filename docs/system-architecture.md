...
**Location**: `.cursor/` and `.opencode/` directories
...
**Hook Configuration** (`.cursor/settings.json`):
```json
{
  "hooks": {
    "BeforeBash": [{
      "type": "command",
      "command": "node ${CLAUDE_PROJECT_DIR}/.cursor/hooks/scout-block.js"
    }]
  }
}
```
...
1. Create agent definition file: `.cursor/agents/my-agent.md`
...
1. Create command file: `.cursor/commands/my-command.md`
...
1. Define workflow in `.cursor/workflows/`
